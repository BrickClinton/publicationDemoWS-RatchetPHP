...
// Obtener un nuevo array de archivos soportados
function getFileArraySupported(files, supportedFiles){
  let newFiles = [];
  
  for (var i = 0; i < files.length; i++) {
    if(validateSupportedFileExtension(files[i], supportedFiles)){
...
function validateSupportedFileExtension(file, supportedFiles){
  // file.type == -1: no encontrado (no soportado)
